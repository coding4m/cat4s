...
trait TraceSet {
  ...
  def withContext[T](name: String, source: TraceSource, traceId: Option[String], parentId: Option[String])(f: TraceContext => T): T = trace(name, source).withTraceId(traceId).withParentId(parentId).collect(f)
